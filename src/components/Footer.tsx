...
export default function Footer() {
  return (
    <footer className="bg-brand-deep border-t border-brand-text/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid md:grid-cols-4 gap-8 mb-8">
          ...
          {/* Solutions */}
          <div>
            <h3 className="text-white font-heading font-bold mb-4">Solutions</h3>
            <ul className="space-y-2">
        ...
        {/* Bottom bar */}
        <div className="border-t border-brand-text/10 pt-8 flex flex-col md:flex-row justify-between items-center">
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <li>
      <Link 
        href={href} 
        className="text-brand-text hover:text-brand-glow transition-colors duration-200 text-sm"
      >
        {children}
      </Link>
    </li>
...
function SocialLink({ href, children, 'aria-label': ariaLabel }: { href: string; children: React.ReactNode; 'aria-label': string }) {
  return (
    <a 
      ...
      className="text-brand-text hover:text-brand-glow transition-colors duration-200"
      target="_blank"
